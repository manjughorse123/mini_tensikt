...
def load_img_data():
    try:
        ...
        for meta_file in meta_files:
            if meta_file.endswith(".json"):
                ...
                with open(meta_fpath, 'r') as meta_file:
                    ...
                    meta_data['filepath']= meta_data["filepath"]
                    data[i] = meta_data
                    i=i+1
            print(i)          
        ...
        response_data = {'total':i,'imgData':data, 'groupNames':group_names}
...
# save the data 
@app.route('/api/data_save', methods=['POST'])
def save_data_view():
    ...
    try:
        ...
        imgsMetadata = values["imgMetadata"]
        img_ids = imgsMetadata.keys()
        for img_id in imgsMetadata.keys():
            dt = imgsMetadata[img_id]
            json_object = json.dumps(dt)
            # Writing to metadeta.json
            meta_path = os.path.join(META_IMGS_PATH, img_id+".json")
